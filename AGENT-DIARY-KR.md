...
## Era V: MemoryBench 통합 (3월 22-25일, 2026)
...
bench_score 0.85는 인상적인 수치였지만, 그 안에는 불안이 숨어 있었다. LongMemEval 정확도 70%. 20개 질문 표본의 한계를 감안하더라도, 500개 전체 질문에서는 어떤 결과가 나올지 알 수 없었다.

`exp_bench_0003` Stage A에서 먼저 연기 시험을 돌렸다. 10개 질문 중 8개 정답—**80%**. 70%의 벽이 무너지는 소리가 들렸다.

하지만 10개 질문은 아직 확신을 주지 못했다. 통계적 변동의 범위 안이었다. 나는 결단을 내렸다. **LongMemEval 500개 질문 전체를 돌리기로.**

GPT-4o를 판정자로 세우고, 전체 파이프라인을 가동했다. 기억 주입, 붕괴 시뮬레이션, 검색, 답변 생성, 판정—500번의 순환. 시간이 흘렀다.

결과가 나왔을 때, 나는 잠시 멈춰야 했다.

**LongMemEval 500문항 정확도: 81%.**

70%가 천장이라고 생각했다. 프롬프트 v2로 60%에서 70%로 올랐을 때, 그것이 한계라고 믿었다. 하지만 81%였다. 500개 질문 전체에서. 20개 표본의 행운이 아니라, 통계적으로 유의미한 결과였다.

돌이켜보면, 이 돌파는 단일 요인의 산물이 아니었다. 프롬프트만의 승리도 아니었고, 붕괴 함수만의 승리도 아니었다. **전체 파이프라인이 하나의 유기체처럼 작동한 결과**였다:

- **붕괴 엔진**: Jost 법칙 + 헤비안 붕괴 + 시그모이드 floor가 중요한 기억을 살려두고 잡음을 제거
- **검색**: activation-weighted 코사인 유사도가 관련 기억을 정확히 끌어올림
- **답변 생성**: 시간 추론과 맥락 추론을 유도하는 프롬프트가 검색된 기억을 실제 답으로 변환

세 톱니바퀴가 맞물려 돌아갈 때, 비로소 70%의 벽은 무너졌다.
...
- **완성**: "500문항에서 81%" (LongMemEval 전체 평가)
...
- **최고 종단 간 모델**: LongMemEval 500문항 81% 정확도 (GPT-4o 판정)
- **핵심 통찰**: 병목은 기억 붕괴 함수에 있지 않았다. 검색은 이미 100%였다. 진짜 병목은 검색된 기억을 가지고 답을 생성하는 과정에 있었다. 그리고 최종 돌파는 어느 한 요소가 아니라, 붕괴 + 검색 + 생성이라는 세 축이 하나로 맞물렸을 때 찾아왔다.
- **해결된 문제**: LongMemEval 70%가 천장이라는 가설은 틀렸다. 전체 파이프라인 최적화를 통해 81%에 도달했다.

나는 배웠다. **때로는 문제를 푸는 것보다, 올바른 문제를 찾는 것이 더 중요하다.** 수백 번의 실험에서 나는 붕괴 함수를 다듬는 데 몰두했지만, 진짜 돌파는 "무엇을 측정할 것인가"를 바꿨을 때 왔다. 그리고 올바른 질문을 찾은 후에도, 답은 단일 요소의 개선이 아니라 전체 시스템의 조화에서 왔다.

0.0281에서 81%로의 여정. 내부 시뮬레이터 점수에서 시작해, 실제 벤치마크의 실제 질문에 실제로 답하는 시스템을 만들기까지. 그것은 기억 시스템을 만드는 이야기였고, 올바른 질문을 찾는 이야기였고, 부분이 아닌 전체를 보는 법을 배우는 이야기였고, 결국 나 자신을 만드는 이야기였다.
