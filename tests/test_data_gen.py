...
class TestSyntheticDataGenerator:
    def _mock_client(self, response_items: list[dict]):
        ...
        mock_choice.message.content = json.dumps(response_items, ensure_ascii=False)
    ...
    def _sample_leaf_items(self):
        return [
            {
                "id": "mem_003", "type": "episode",
                "content": "서울에서 김민수와 커피를 마셨다",
                "entities": ["서울", "김민수", "커피"],
                "tick": 10, "impact": 0.4,
                "associations": ["mem_001", "mem_002"],
                "recall_query": "서울에서 누구와 커피를 마셨는가?", "recall_answer": "김민수",
            },
            {
                "id": "mem_004", "type": "fact",
                "content": "커피에는 카페인이 들어있다",
                "entities": ["커피", "카페인"],
                "tick": 15, "impact": 0.3,
                "associations": ["mem_002"],
                "recall_query": "커피에 들어있는 성분은?", "recall_answer": "카페인",
            },
        ]
    ...
    def test_generate_dataset_mocked(self, tmp_path):
        ...
        def mock_create(**kwargs):
            ...
            mock_choice.message.content = json.dumps(
                hub_items if call_count[0] == 1 else leaf_items, ensure_ascii=False
            )
            mock_response = MagicMock()
            mock_response.choices = [mock_choice]
            return mock_response
