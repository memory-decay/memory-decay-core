"""Jost decay with a bounded impact-stability interaction term."""
...
def compute_decay(activation, impact, stability, mtype, params):
    ...
    interaction_weight = params.get("interaction_weight", 0.20)

    base_importance = (impact * alpha + stability * rho) / (alpha + rho)
    synergy = impact * stability
    importance = base_importance + interaction_weight * synergy * (1.0 - base_importance)
    ...
    retention = max(1.0 + alpha * impact + rho * stability + interaction_weight * synergy, 1.0)
