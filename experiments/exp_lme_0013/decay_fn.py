"""Jost's Law decay: stability only affects floor, not retention."""
...
def compute_decay(activation, impact, stability, mtype, params):
    ...
    # --- Floor: sigmoid of impact, with stability as multiplier ---
    ...
    # Stability boosts floor but does NOT affect retention/decay
    stability_boost = 1.0 + rho * stability

    floor = floor_max * sigmoid * stability_boost
    floor = min(floor, activation)

    # --- Jost decay: retention only from impact (not stability) ---
    ...
    # Retention from impact only - stability handled separately via floor
