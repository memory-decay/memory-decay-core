"""Jost decay with a softer near-floor phase."""
...
def compute_decay(activation, impact, stability, mtype, params):
    ...
    transition = params.get("transition_excess", 0.18)
    jost_power_high = params.get("jost_power_high", 4.0)
    jost_power_low = params.get("jost_power_low", 2.4)

    if excess > transition:
        decay_amount = effective_lambda * (excess ** jost_power_high)
    else:
        high_term = transition ** max(jost_power_high - jost_power_low, 0.0)
        decay_amount = effective_lambda * high_term * (excess ** jost_power_low)
