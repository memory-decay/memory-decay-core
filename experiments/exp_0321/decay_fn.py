"""Jost decay with stability-dependent floor and asymmetric fact/episode."""
...
def compute_decay(activation, impact, stability, mtype, params):
    ...
    # Importance includes stability more heavily
    importance = (impact * alpha + stability * rho * 1.5) / (alpha + rho * 1.5)

    # Floor: higher for stable+important memories
    ...
    # Asymmetric fact/episode: facts decay 3x slower
    lambda_fact = params.get("lambda_fact", 0.006)
    lambda_episode = params.get("lambda_episode", 0.035)
    base_lambda = lambda_fact if mtype == "fact" else lambda_episode

    # Retention: stability provides quadratic protection
    retention = max(1.0 + alpha * impact + rho * stability * (1.0 + stability), 1.0)
