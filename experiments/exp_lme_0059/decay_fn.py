...
Hypothesis: impact=0 items need floor=0 to pass strict validator (fact must decay < 0.30 at tick 200).
The sigmoid floor for zero-impact items is too high because sigmoid(importance=0) still gives a small positive floor.

Fix: When impact=0, bypass the sigmoid floor entirely and use pure exponential decay with
a moderate lambda so activation drops to ~0.1-0.2 by tick 200.
"""
...
def compute_decay(activation, impact, stability, mtype, params):
    ...
    if impact <= 0:
        ...
        # Use higher lambda so it decays to < 0.30 at tick 200
        # 0.5 * exp(-0.025 * 200) = 0.5 * exp(-5) ≈ 0.0034
        effective_lambda = base_lambda * 3.0  # ~0.027 for fact
        new_activation = activation * math.exp(-effective_lambda)
        return max(0.0, new_activation)

    # --- Normal items: Jost's Law with sigmoid floor ---
